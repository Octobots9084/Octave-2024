...
public class SwerveMath {
  ...
  /**
   * Calculate the meters per rotation for the integrated encoder. Calculation:
   * (PI * WHEEL DIAMETER IN METERS) / (GEAR
   * RATIO * ENCODER RESOLUTION)
   *
   * @param wheelDiameter    Wheel diameter in meters.
   * @param driveGearRatio   The gear ratio of the drive motor.
   * @param pulsePerRotation The number of encoder pulses per rotation. 1 if using
   ...
   * @param scaled   Use algebra to determine deadband by starting the value at 0
   *                 past deadband.
   ...
   *
   * @param optimalVoltage                 Optimal voltage to calculate kV
   *                                       (voltage/max Velocity)
   * @param maxSpeed                       Maximum velocity in meters per second
   *                                       to use for the feed forward, should be
   *                                       as close to physical max as possible.
   * @param wheelGripCoefficientOfFriction Wheel grip coefficient of friction for
   *                                       kA (voltage/(cof*9.81))
  ...
  public static SimpleMotorFeedforward createDriveFeedforward(double optimalVoltage, double maxSpeed,
    ...
    double kv = 0.1 * (optimalVoltage / maxSpeed);
    /// ^ Volt-seconds per meter (max voltage divided by max speed)
    double ka = 0.7 * (optimalVoltage
   ...
   * Encoder conversion values. Drive converts
   * motor rotations to linear wheel distance and steering converts motor
   * rotations to module azimuth.
   *
   * @param angleGearRatio   The gear ratio of the steering motor.
   * @param pulsePerRotation The number of pulses in a complete rotation for the
   *                         encoder, 1 if integrated.
  ...
  /**
   * Calculate the practical maximum acceleration of the robot using the wheel
   ...
   * @param gearRatio     Gear ratio for driving motor number of motor rotations
   *                      until one wheel rotation.
  ...
  /**
   * Calculates the maximum acceleration allowed in a direction without tipping
   * the robot. Reads arm position from
   * NetworkTables and is passed the direction in question.
   *
   * @param angle     The direction in which to calculate max acceleration, as a
   *                  Rotation2d. Note that this is
   *                  robot-relative.
   * @param matter    Matter that the robot is composed of in kg. (Includes
   *                  chassis)
   * @param robotMass The weight of the robot in kg. (Including manipulators,
   *                  etc).
  ...
  private static double calcMaxAccel(
    ...
    // Calculate the vertical mass moment using the floor as the datum. This will be
    // used later to
  ...
  /**
   * Limits a commanded velocity to prevent exceeding the maximum acceleration
   * given by {@link SwerveMath#calcMaxAccel}.
   * Note that this takes and returns field-relative velocities.
   *
   * @param commandedVelocity The desired velocity
   * @param fieldVelocity     The velocity of the robot within a field relative
   *                          state.
   * @param robotPose         The current pose of the robot.
   * @param loopTime          The time it takes to update the velocity in seconds.
   *                          <b>Note: this should include the
   *                          100ms that it takes for a SparkMax velocity to
   *                          update.</b>
   * @param matter            Matter that the robot is composed of with position
   *                          in meters and mass in kg.
   * @param robotMass         The weight of the robot in kg. (Including
   *                          manipulators, etc).
   * @param config            The swerve drive configuration.
   * @return The limited velocity. This is either the commanded velocity, if
   *         attainable, or the closest attainable
   *         velocity.
   */
   ...
   * @return Module location which is the furthest from center and abides by
   *         parameters.
   */
  ...
  /**
   * Put an angle within the 360 deg scope of a reference. For example, given a
   * scope reference of 756 degrees, assumes
   * the full scope is (720-1080), and places an angle of 22 degrees into it,
   * returning 742 deg.
   *
